...
double twist_angle_calculation(struct adaptive_frame fr1, struct adaptive_frame fr2);

void derivative_of_twist_bend(int in, adaptive_frame *Mf, float leng1, float leng2, float gram[2][3], float hesm[4][3][3], float grke[2][2][3],
	float heske[2][4][3][3]);
...
//rod energyの最小化
bool curve_on_mesh::minimize_discrete_rod_energy(AABB_struct *aabb,int maxIter){
	...
	float(*duv)[2][3],(*grt)[3],(*hest)[3][3];
	duv = new float[nodeN][2][3];
	grt = new float[nodeN - 1][3];
	hest = new float[nodeN - 1][3][3];
...
#pragma omp parallel for
		for (int i = 0; i < nodeN; i++){
			...
			if (i == nodeN - 1) continue;
			/*float cent[3];
			for (int j = 0; j < 3; j++){
				cent[j] = 0.5*(cNode[i].pos[j] + cNode[i + 1].pos[j]);
			}
			calc_derivative_of_twist(i, Mframe, cent, grt[i], hest[i]);*/
		}
		for (int i = 0; i < nodeN; i++){
			if (pCons[i].cType != -1){
				float leng = 0.0;
		...
		for (int i = 1; i < nodeN - 1; i++){
			float mtw = twist_angle_calculation(Mframe[i - 1], Mframe[i]);
			float kb[3];
			float kai = 1.0;
			for (int j = 0; j < 3; j++){
			...
			float gram[2][3], hesm[4][3][3];
			derivative_of_twist_bend(i, Mframe, leng1, leng2, gram, hesm, gke, hkee);
			...
			/*for (int j = 0; j < 3; j++){
				Fi[j] += kt*mtw*gram[0][j] / leni;
				Fi[3 + j] += kt*mtw*(-gram[0][j] + gram[1][j]) / leni;
				Fi[6 + j] += -kt*mtw*gram[1][j] / leni;

				Fi[j] += 0.5*kt*mtw*grt[i - 1][j] / leni;
				Fi[3 + j] += 0.5*kt*mtw*(-grt[i][j] + grt[i - 1][j]) / leni;
				Fi[6 + j] += -0.5*kt*mtw*grt[i][j] / leni;
				for (int k = 0; k < 3; k++){
					Ki[j][k] += 0.25*kt*(grt[i - 1][j] * grt[i - 1][k] - mtw*hest[i - 1][j][k]) / leni;
					Ki[3 + j][3 + k] += 0.25*kt*(grt[i - 1][j] * grt[i - 1][k] - mtw*hest[i - 1][j][k] + grt[i][j] * grt[i][k] + mtw*hest[i][j][k]
						- grt[i][j] * grt[i - 1][k] - grt[i - 1][j] * grt[i][k]) / leni;
					Ki[6 + j][6 + k] += 0.25*kt*(grt[i][j] * grt[i][k] + mtw*hest[i][j][k]) / leni;
					Ki[j][3 + k] += 0.25*kt*(grt[i - 1][j] * grt[i - 1][k] - grt[i - 1][j] * grt[i][k] - mtw*hest[i - 1][j][k]) / leni;
					Ki[3 + j][k] += 0.25*kt*(grt[i - 1][j] * grt[i - 1][k] - grt[i][j] * grt[i - 1][k] - mtw*hest[i - 1][j][k]) / leni;
					Ki[j][6 + k] += -0.25*kt*grt[i - 1][j] * grt[i][k] / leni;
					Ki[6 + j][k] += -0.25*kt*grt[i][j] * grt[i - 1][k] / leni;
					Ki[3 + j][6 + k] += 0.25*(-grt[i - 1][j] * grt[i][k] + grt[i][j] * grt[i][k] + mtw*hest[i][j][k]) / leni;
					Ki[6 + j][3 + k] += 0.25*(-grt[i][j] * grt[i - 1][k] + grt[i][j] * grt[i][k] + mtw*hest[i][j][k]) / leni;

					for (int l = 0; l < 2; l++){
						for (int m = 0; m < 2; m++){
							float val = kt*(gram[l][j] * gram[m][k] + mtw*hesm[2 * l + m][j][k]) / leni;
							Ki[3 * l + j][3 * m + k] += val;
							Ki[3 * l + j][3 * (m + 1) + k] -= val;
							Ki[3 * (l + 1) + j][3 * m + k] -= val;
							Ki[3 * (l + 1) + j][3 * (m + 1) + k] += val;
						}
					}
				}
			}*/
		...
		for (int i = 0; i < nodeN - 1; i++){
			if (dCons[i].cType != -1){
				...
				float gT[3],KT[3][3];
				for (int j = 0; j < 3; j++){
					if (fabs(sin(theta)) > 1.0e-3) gT[j] = -(dCons[i].pos[j] - seki / leng / leng*ed[j]) / leng / sin(theta);
					else gT[j] = 0.0;
				}
				for (int j = 0; j < 3; j++){
					for (int k = 0; k < 3; k++){
						if (fabs(sin(theta))>1.0e-3){
...
#pragma omp parallel for
		...
		//printf("%d E:%f %f %f f:%f move:%f\n",iterN,Ebend,Etwis,Ediff,sqrt(fval),max_move);
		printf("%d E:%f(%f %f %f %f,%f) move:%f\n",iterN,Ebend+Ediff+Econs+Etwis,Ebend,Etwis,Ediff,Econs,Edcons,max_move);
...
#pragma omp parallel for
	for (int i = 0; i < nodeN - 1; i++){
		...
		float seki = oseki[0] * Mframe[i].d3[0] + oseki[1] * Mframe[i].d3[1] + oseki[2] * Mframe[i].d3[2];
		if (theta > 1.0) theta = 1.0;
		if (theta < -1.0) theta = -1.0;
		theta = acos(theta);
		...
		if (Phik > Phi0 + c1*alph*dPhi0 || (iterN > 0 && Phik > Phik1)){
			...
			while (1){
				...
				if ((al_lo - alpj)*(al_hi - alpj) > 0 || fabs(al_lo - alpj) < 0.01*fabs(al_lo - al_hi)
					|| fabs(al_hi - alpj) < 0.01*fabs(al_lo - al_hi) || _isnan(alpj)){
...
#pragma omp parallel for
	...
	for (int i = 0; i < nodeN - 1; i++){
		if (dCons[i].cType != -1){
			...
			//for (int j = 0; j < 3; j++){
			//	float fval = 0.0;
			//	if (fabs(sin(theta)) > 1.0e-3) fval = -(dCons[i].pos[j] - seki / leng / leng*ed[j]) / leng / sin(theta);
			//	gE[j] = -mu2*theta*fval;
...
//		float cent[3];
//		for (int j = 0; j < 3; j++){
//			cent[j] = 0.5*(Xk(3 * i + j) + Xk(3 * i + 3 + j));
//		}
//		calc_derivative_of_twist(i, Mframe, cent, grt[i], hest[i]);
//	}
	...
	for (int i = 1; i < nodeN - 1; i++){
		...
		/*for (int j = 0; j < 3; j++){
			gE[j] += -kt*mtw*gram[0][j] / leni;
			gE[3 + j] += -kt*mtw*(-gram[0][j] + gram[1][j]) / leni;
			gE[6 + j] += kt*mtw*gram[1][j] / leni;

			gE[j] += -0.5*kt*mtw*grt[i - 1][j] / leni;
			gE[3 + j] += -0.5*kt*mtw*(-grt[i][j] + grt[i - 1][j]) / leni;
			gE[6 + j] += 0.5*kt*mtw*grt[i][j] / leni;
		}*/
...
void curve_on_mesh::remesh_curve_node(int selectC,int &selectN, int &selectE, class AABB_struct *AABB,std::vector<struct ConnectData> &connect){
	...
	int maxNodeN = 200;
	float minLeng = sParam->Wid*1.5;
	float maxLeng = sParam->Wid*3.0;
	...
	while (1){
		...
		if(!minimize_discrete_rod_energy(AABB,20)) return;
	}
	...
	while (1){
		...
		if (maxN == -1 || nodeN == maxNodeN) break;
		if (maxL < maxLeng) break;
...
void curve_on_mesh::calc_derivative_of_twist(int eN, adaptive_frame *mfr, float cent[3],float grm[3],float hesm[3][3]){


	float dS[2][3], ddS[2][2][3], dddS[2][2][2][3];

	int fn;
	float cord[3];
	bool success = laabb->closest_point_search_with_mesh_info(cent, &fn, cord);
	if (!success) {
		printf("closest point not exist\n");
		return;
	}
	lmesh->evaluate_derivative_of_box_spline_on_triangle(fn, cord, dS);
	lmesh->evaluate_second_derivative_of_box_spline_on_triangle(fn, cord, ddS);
	lmesh->evaluate_third_derivative_of_box_spline_on_triangle(fn, cord, dddS);

	float Sx[3], P[3][3];
	for (int i = 0; i < 3; i++){
		Sx[i] = dS[0][(i + 1) % 3] * dS[1][(i + 2) % 3] - dS[0][(i + 2) % 3] * dS[1][(i + 1) % 3];
		for (int j = 0; j < 3; j++){
			P[i][j] = -mfr[eN].d3[i] * mfr[eN].d3[j];
			if (i == j) P[i][j] += 1.0;
		}
	}
	float PSxh = 0.0;
	...
	float gS[2][3], hS[2][2][3];
	for (int i = 0; i < 2; i++){
		for (int j = 0; j < 3; j++){
			gS[i][j] = ddS[0][i][(j + 1) % 3] * dS[1][(j + 2) % 3] + dS[0][(j + 1) % 3] * ddS[1][i][(j + 2) % 3]
				- ddS[0][i][(j + 2) % 3] * dS[1][(j + 1) % 3] - dS[0][(j + 2) % 3] * ddS[1][i][(j + 1) % 3];
		}
		for (int j = 0; j < 2; j++){
			for (int k = 0; k < 3; k++){
				hS[i][j][k] = dddS[0][i][j][(k + 1) % 3] * dS[1][(k + 2) % 3] + ddS[0][i][(k + 1) % 3] * ddS[1][j][(k + 2) % 3]
					+ ddS[0][j][(k + 1) % 3] * ddS[1][i][(k + 2) % 3] + dS[0][(k + 1) % 3] * dddS[1][i][j][(k + 2) % 3]
					- dddS[0][i][j][(k + 2) % 3] * dS[1][(k + 1) % 3] - ddS[0][i][(k + 2) % 3] * ddS[1][j][(k + 1) % 3]
					- ddS[0][j][(k + 2) % 3] * ddS[1][i][(k + 1) % 3] - dS[0][(k + 2) % 3] * dddS[1][i][j][(k + 1) % 3];
			}
		}
	}
	...
	for (int i = 0; i < 2; i++){
		float seki = 0.0;
		for (int j = 0; j < 3; j++){
			seki += SPP[j] * gS[i][j] / pow(PSxh, 3);
		}
		for (int j = 0; j < 3; j++){
			gd2[i][j] = 0.0;
			for (int k = 0; k < 3; k++){
				gd2[i][j] += P[j][k] * gS[i][k] / PSxh - seki*P[j][k] * Sx[k];
			}
		}
	}
	...
	for (int i = 0; i < 2; i++){
		for (int j = 0; j < 2; j++){
			float seki = 0.0;
			for (int k = 0; k < 3; k++){
				seki += SPP[k] * hS[i][j][k] / pow(PSxh, 3);
				for (int l = 0; l < 3; l++){
					for (int m = 0; m < 3; m++){
						seki += gS[i][k] * P[l][k] * P[l][m] * gS[j][m] / pow(PSxh, 3);
					}
				}
			}
			for (int k = 0; k < 3; k++){
				hd2[i][j][k] = 0.0;
				for (int l = 0; l < 3; l++){
					hd2[i][j][k] += P[k][l] * hS[i][j][l] / PSxh - SPPgS[i] * P[k][l] * gS[j][l] / pow(PSxh, 3)
						- SPPgS[j] * P[k][l] * gS[i][l] / pow(PSxh, 3) - seki*P[k][l] * Sx[l]
						+ 3.0*SPPgS[i] * SPPgS[j] * P[k][l] * Sx[l] / pow(PSxh, 5);
				}
			}
		}
	}
	...
	for (int i = 0; i < 2; i++){
		gte[i] = 0.0;
		for (int j = 0; j < 3; j++){
			gte[i] += -mfr[eN].d1[j] * gd2[i][j];
		}
		for (int j = 0; j < 2; j++){
			hte[i][j] = 0.0;
			for (int k = 0; k < 3; k++){
				hte[i][j] += -mfr[eN].d1[k] * hd2[i][j][k];
			}
		}
	}
	...
	//uvからXYZへの変換
	Eigen::MatrixXf Jac(3, 3), invJ(3, 3);
	for (int i = 0; i < 3; i++){
		Jac(i, 0) = dS[0][i];
		Jac(i, 1) = dS[1][i];
		Jac(i, 2) = Sx[i];
	...
	for (int i = 0; i < 3; i++){
		float seki = 0.0;
		for (int j = 0; j < 3; j++){
			seki += mfr[eN].d3[j] * invJ(i, j);
		}
		for (int j = 0; j < 3; j++){
			invJ(i, j) -= seki*mfr[eN].d3[j];
		}
	}
	...
	for (int i = 0; i < 3; i++){
		grm[i] = 0.0;
		for (int j = 0; j < 2; j++){
			grm[i] += gte[j] * invJ(j, i);
		}
		for (int j = 0; j < 3; j++){
			hesm[i][j] = 0.0;
			for (int k = 0; k < 2; k++){
				for (int l = 0; l < 2; l++){
					hesm[i][j] += hte[k][l] * invJ(k, i)*invJ(l, j);
				}
			}
		}
	}
...
bool CurveNetwork::exportCurveNet(std::string fname, class AABB_struct *AABB){
	...
	FILE *fp;
	errno_t error;
	if ((error = fopen_s(&fp, fname.c_str(), "w")) != 0){
		return false;
	}
